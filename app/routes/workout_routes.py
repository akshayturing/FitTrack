...
def create_workout():
    ...
    try:
        ...
        current_app.logger.error(f"Failed to create workout: {str(e)}")
        return jsonify({'error': 'Failed to create workout'}), 500

@workout_bp.route('/me', methods=['GET'])
@jwt_required()
def get_my_workouts():
    """Get workouts of the currently authenticated user"""
    # Get the current user's ID from the JWT
    user_id = get_jwt_identity()
...
def get_user_workouts_api(user_id):
    ...
    try:
        ...
        # For debugging
        current_app.logger.info(f"JWT user_id: {current_user_id}, Requested user_id: {user_id}")
        
        # Get the authenticated user to check for admin status
        current_user = User.query.get(current_user_id)
        if not current_user:
            current_app.logger.error(f"User with ID {current_user_id} not found")
            return jsonify({"error": "Authenticated user not found"}), 404
        
        # Check if the user is requesting their own workouts or is an admin
        is_admin = getattr(current_user, 'is_admin', False)
        current_app.logger.info(f"User is admin: {is_admin}")
        
        # IMPORTANT FIX: Allow access if it's the same user or an admin
        # Convert IDs to integers to ensure proper comparison
        if int(current_user_id) != int(user_id) and not is_admin:
            current_app.logger.warning(f"Unauthorized access attempt: User {current_user_id} trying to access workouts of user {user_id}")
            return jsonify({"error": "You are not authorized to access these workouts"}), 403
        
        # Check if requested user exists
        requested_user = User.query.get(user_id)
        if not requested_user:
            current_app.logger.error(f"Requested user with ID {user_id} not found")
            return jsonify({"error": "Requested user not found"}), 404
            
        # Get all workout assignments for the user
        assignments = WorkoutAssignment.query.filter_by(user_id=user_id).all()
        ...
        current_app.logger.info(f"Found {len(workouts)} workouts for user {user_id}")
...
def get_workout(workout_id):
    ...
    try:
        ...
        return jsonify({"error": f"Failed to fetch workout: {str(e)}"}), 500
